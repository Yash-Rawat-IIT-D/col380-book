...
\begin{pppitfall}
Work and cost are not the same quantity in every case. A reduction tree performs only $\Theta(n)$
additions, but if too many processors sit idle across too many rounds, the total
cost can still be $\Theta(n \log n)$. Thus a better approach is to free half of the processors after each round
, so that the cost remains $\Theta(n)$. 
\end{pppitfall}
...
the exclusive result. As shown in Figure~\ref{fig:blelloch-scan}, the upsweep
...
Tree-based sum reduction & $\Theta(\log n)$ & $\Theta(n)$ & Yes \\
